...
if (!$productModel->getIdBySku('sku-book')) {
    $productModel
            ...
            ->setSku('sku-donwloadable')
            ->setName('Ion Creanga')
            ->setCreatedAt(strtotime('now'))
            ->setDescription('Ion Creanga')
            ->setShortDescription('Ion Creanga')
            ->setPrice(30)
            ->setTaxClassId(0)
    ...
    $linkModel = Mage::getModel('downloadable/link')
            ...
            ->setLinkUrl('')
            ->setLinkType('file')
            ->setLinkFile('/evozon/iubita.jpg')
            ->save();
}
Mage::log('Ended data-upgrade-0.1.7', null, 'scripts.log');
