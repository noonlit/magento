...
try {
    ...
    if ($installer->getConnection()->isTableExists($tableName) != true) {
        $table = $installer->getConnection()
            ...
            ->addColumn(
                'question', Varien_Db_Ddl_Table::TYPE_TEXT, 255, array(
                    'nullable' => false
                    ), 'Question text'
            )
            ->addColumn(
            ...
            ->addColumn(
                'customer_name', Varien_Db_Ddl_Table::TYPE_TEXT, 100, array(
                    'unsigned' => true,
                    'nullable' => false,
                    ), 'Customer Name'
            )
...
try {
    ...
    if ($installer->getConnection()->isTableExists($tableName) != true) {
        $table = $installer->getConnection()
            ->newTable($tableName)
            ->addColumn(
                'answer_id', Varien_Db_Ddl_Table::TYPE_INTEGER, null, array(
                    'primary' => true,
                    'nullable' => false,
                    'unsigned' => true,
                    'identity' => true,
            ...
            ->addColumn(
                'admin_id', Varien_Db_Ddl_Table::TYPE_INTEGER, null, array(
                    'nullable' => false,
                    'unsigned' => true,
                    )
            )
            ->addColumn(
                'answer', Varien_Db_Ddl_Table::TYPE_TEXT, 255, array(
                    'nullable' => false
                    )
            )
            ->addColumn(
            ...
            ->addForeignKey(
                ...
                ), 'question_id', $installer->getTable('evozon_qa/question'), 'question_id', 
                Varien_Db_Ddl_Table::ACTION_CASCADE, Varien_Db_Ddl_Table::ACTION_CASCADE
            )
            ->addForeignKey(
                $installer->getFkName(
